...
void	draw_point(t_data *data, int x, int y)
{
	...
	while (i < MAP_SIZE)
	{
		...
		while (j < MAP_SIZE)
		{
			if (x + j > data->img_width || y + i > data->img_height || x < 0
				|| y < 0)
				break ;
			radius = MAP_SIZE / 4;
			dx = j - MAP_SIZE / 2;
			...
			if (dx * dx + dy * dy <= radius * radius)
				data->img_addr[(y + i) * data->img_width + (x + j)] = 0xff0000;
			j++;
		}
		i++;
	}
	...
	while (i < MAP_SIZE)
	{
		...
		while (j < MAP_SIZE)
		{
			...
			if (i == 0 || i == MAP_SIZE - 1 || j == 0 || j == MAP_SIZE - 1)
				data->img_addr[(y + i) * data->img_width + (x + j)] = 0x000000;
			else
				data->img_addr[(y + i) * data->img_width + (x
						+ j)] = data->floor_color;
			j++;
		}
		i++;
